...
describe.skip("cleanString", () => {
  const collectionFiles = corpus.collections.flatMap((collectionKey) =>
    corpus.getCollection(collectionKey).files.map((fileKey) =>
      [collectionKey, fileKey] as const));

  test.concurrent.each(collectionFiles)(
    "%s > %s", async (collection, file) => {
      ...
      postprocess.forEach((lines) => {
        lines.forEach((s) => {
          expect(s).not.toMatch(/[\uE000-\uF8FF\u{F0000}-\u{FFFFD}\u{100000}-\u{10FFFD}]/u); // Check for private use characters
        });
      });
    },
    1000 * 60 * 5); // 5 minutes

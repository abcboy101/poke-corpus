import { CollectionKey, Corpus, LanguageKey } from "./corpus";
...
const defaultSearchParams: Required<Omit<SearchParamsURL, 'collections' | 'languages'>> = {
  id: '',
  file: '',
  query: '',
  type: 'all',
...
const searchParamsFactory = (corpus: Corpus) => {
   ...
   * If the parameter is invalid or missing, that field will be undefined.
   */
  function queryStringToSearchParams(hash: string): Partial<SearchParamsURL> {
    ...
    return {
      ...
      collections: asOptionalArray(params.get('collections'), corpus.isCollectionKey),
      languages: asOptionalArray(params.get('languages'), corpus.isLanguageKey, remapLanguages),
  ...
  */
   ...
   */
  function searchParamsToBase64(params: SearchParamsURL) {
    const urlOnlyString: Record<string, string> = {};
    if (params.id !== undefined)
      urlOnlyString.id = params.id;
    if (params.file !== undefined)
   ...
   */
  function base64ToSearchParams(hash: string): Partial<SearchParamsURL> {
   ...
   */
  function deserializeByteArray(bytes: ReadonlyUint8Array): SearchSettings {
  ...
  function decryptBytes(bytes: Uint8Array) {
  ...
  return {
    // protected (exposed for unit tests)
    _bytesBase64: bytesBase64,
    _searchParamsToQueryString: searchParamsToQueryString,
    _queryStringToSearchParams: queryStringToSearchParams,
    _searchParamsToBase64: searchParamsToBase64,
    _base64ToSearchParams: base64ToSearchParams,
    _serializeByteArray: serializeByteArray,
    _deserializeByteArray: deserializeByteArray,
...
export type SearchParamsFactory = ReturnType<typeof searchParamsFactory>;

export function getSearchParamsFactory(corpus: Corpus): SearchParamsFactory {
  return searchParamsFactory(corpus);
}
